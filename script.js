...
function GenerateCard(){
    ...
    html2canvas($("#report-card-HTML"), { // DIV ID HERE
        onrendered: function(canvas) {
            ...
            doc.save('report-card-generator.pdf'); //SAVE PDF FILE
        }
    });
...
function print_report_card() {
    // First, ensure all data from the form is correctly updated on the report card
    send_data_for_print(); // This will update the report card with the latest input data

    // Prepare the report card for printing
    var printContent = document.getElementById('report-card-HTML').innerHTML;
    var originalContent = document.body.innerHTML;

    document.body.innerHTML = printContent; // Set the body's content to just the report card's HTML for printing

    window.print(); // Open print dialog

    document.body.innerHTML = originalContent; // Restore the original content to the page after printing
}
